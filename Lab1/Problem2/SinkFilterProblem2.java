...
public class SinkFilterProblem2 extends FilterFramework
{
	public void run()
    {
		...
		/************************************************************************************
		 *	We want to designate the location of where to write our csv file to.
		 *	Using FileWriter to directly write our data into a csv file.
		 *************************************************************************************/
		String filePath = "OutputB.csv";
		...
		*************************************************************************************/
		...
		/************************************************************************************
		 *	We now set up the header of our CSV File.
		 *************************************************************************************/
		try{
			csvWriter.append("Time,");
			csvWriter.append("Velocity,");
			...
			while (true)
			{
				try
				{
					...
					if ( id == 0 )
					{
						...
						csvWriter.append(TimeStampFormat.format(TimeStamp.getTime())); //We write the time to our csv File
						csvWriter.append(",");
					}
					 ...
					 ****************************************************************************/
						...
						if (value < 0 ) {
							strDouble = String.format("%.5f", value * -1)+"*";
						}
						else{
							strDouble = String.format("%.5f",value);
						}
						csvWriter.append(strDouble);
						csvWriter.append(",");

					}
					else if ( id == 1 || id == 3 || id == 4)
					{
						String strDouble = String.format("%.5f", Double.longBitsToDouble(measurement));
						csvWriter.append(strDouble);
						//csvWriter.append( Double.toString(Double.longBitsToDouble(measurement) )); //Write to csv file with appropriate data
						//						csvWriter.append( String.format("%.5f",Double.toString(Double.longBitsToDouble(measurement)) )); //Write to csv file with appropriate data
						/***************************************************************************
						 // If the id is 4, we know we have reached the end of a data set, and would like to add a new line
						 // to represent a new data point. If not, just add a comma to separate data from each other.
						 ****************************************************************************/
